...
class SourceMapURLService {
  ...
  #sourcesLoading;
  #urlToIDMap;
  #onResourceAvailableHandler;
  #syncPrevValueHandler;
  ...
  constructor(commands, sourceMapLoader) {
    ...
    this.#onResourceAvailableHandler = resources =>
      this.#onResourceAvailable(resources);
    this.#syncPrevValueHandler = () => this.#syncPrevValue();

    Services.prefs.addObserver(SOURCE_MAP_PREF, this.#syncPrevValueHandler);
  ...
  destroy() {
    Services.prefs.removeObserver(SOURCE_MAP_PREF, this.#syncPrevValueHandler);
  ...
  #flushPendingIDSubscriptionsToMapQueries(map) {
    ...
    for (const entry of subscriptions) {
      ...
      if (query.mostRecentEmitted) {
        ...
        // #all_ subscribers will have received that value.
    ...
    if (!this.#sourcesLoading) {
      ...
      const onResources = resourceCommand.watchResources(
        ...
        {
          onAvailable: this.#onResourceAvailableHandler,
        }
      );
